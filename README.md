...
```bash
make ci -jN  # where N is the number of parallel jobs (e.g., make ci -j4)  # where N is the number of parallel jobs (e.g., make ci -j4) # where N is the number of max concurrent jobs, or just -j for infinite jobs
```
